...
class EyeTracking:
	...
	def gen_face_points(self):
		...
		for face in faces:
			...
			# array
			self.face_points = predictor(self.img, face)
	...
	def mainloop(self):
		while True:
			...
			if(ret):
				...
				# 	cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
				# show the frame
				cv2.imwrite("eye.jpg", targetROI)
				blur = cv2.GaussianBlur(targetROI, (5,5), 0)
				# thresh_val = r = cv2.getTrackbarPos('threshold', 'thresh')
				_, threshold = cv2.threshold(blur, 35,255, cv2.THRESH_BINARY_INV)
				...
				if len(centers) >2:
					...
					print("areas: ", areas)
					cv2.imwrite("manycenter.jpg", frame)
					break
				if len(centers) != 2:
					continue
				...
				self.data = [self.face_points, self.centers, self.dists]
